...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//cmd0:CMD">
        ...
        <xsl:variable name="profile">
            <xsl:variable name="header">
                <xsl:choose>
                    <xsl:when test="matches(cmd0:Header/cmd0:MdProfile,'.*(clarin.eu:cr1:p_[0-9]+).*')">
                        <xsl:sequence select="replace(cmd0:Header/cmd0:MdProfile,'.*(clarin.eu:cr1:p_[0-9]+).*','$1')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:sequence select="()"/>
                    </xsl:otherwise>
            ...
            <xsl:variable name="schema"> 
                <xsl:variable name="location">
                    <xsl:choose>
                        <xsl:when test="normalize-space(@xsi:noNamespaceSchemaLocation)!=''">
                            ...
                            <xsl:sequence select="normalize-space(@xsi:noNamespaceSchemaLocation)"/>
                        </xsl:when>
                        <xsl:when test="normalize-space(@xsi:schemaLocation)!=''">
                            <xsl:variable name="pairs" select="tokenize(@xsi:schemaLocation,'\s+')"/>
                            <xsl:choose>
        ...
        <cmd:CMD>
            ...
            <xsl:apply-templates select="cmd0:Header">
                ...
                <xsl:with-param name="cmd-profile" select="$cmd-profile" tunnel="yes"/>
                <xsl:with-param name="cr-profile-xml" select="$cr-profile-xml" tunnel="yes"/>
                <xsl:with-param name="cmd-profile-uri" select="$cmd-profile-uri" tunnel="yes"/>
            </xsl:apply-templates>
    ...
    <xsl:template match="cmd0:CMD/cmd0:Header" priority="2">
        ...
        <xsl:param name="cmd-profile" tunnel="yes"/>
        <xsl:param name="cr-profile-xml" tunnel="yes"/>
        <xsl:param name="cmd-profile-uri" tunnel="yes"/>
        <cmd:Header>
    ...
    <xsl:template match="cmd0:CMD/cmd0:Components//*" priority="2">
        ...
        <xsl:param name="base"/>
        <xsl:param name="cmd-profile"/>
        <xsl:param name="cr-profile-xml"/>
        <xsl:param name="cmd-profile-uri" tunnel="yes"/>
